...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>"<<",'next'=>'>>','first'=>'|<','last'=>'>|','theme'=>' <span>%totalRow% %header%</span> <span>%nowPage%/%totalPage% 页</span> %first%  %upPage% %prePage%  %linkPage%  %nextPage%  %downPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function set_page($p,$url){
    	$this->nowPage=$p;
    	if($this->nowPage<1){
            $this->nowPage  =   1;
        }elseif(!empty($this->totalPages) && $this->nowPage>$this->totalPages) {
    ...
    public function show() {
        ...
        //echo $this->url . C('URL_PATHINFO_DEPR');
        // 分析分页参数
        if($this->url){
        ...
        $downRow        =   $this->nowPage+1;
            ...
            if($downRow>=$this->totalPages){$downRow=$this->totalPages;}
