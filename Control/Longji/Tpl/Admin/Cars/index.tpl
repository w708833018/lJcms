...
<form name="data_form" id="data_form" method="POST" action="{:U('')}">
    ...
    <td class="td_center"><a href="{:U('Product/add','sid='.$rs['id'].'&catid='.$rs['catid'])}">产品</a> | <a href="{:U('add','cmd=copy&id='.$rs[id])}" title="复制一个相同产品">复制</a> | <a href="{:U('add','id='.$rs[id])}" title="编辑该模型">编辑</a> | <a href="javascript:void();" onclick="do_del('{$rs.id}');">删除</a></td>
		</tr>
