...
class TagLibLJ extends TagLib
{
	protected $tags   =  array(
		...
		'catpos' => array('attr'=>'catid,space,format','close'=>0),
	...
	public function _subcat($attr,$content) {
		...
		$limit = !empty($tag['limit'])?$tag['limit']:2000;
		$self = !empty($tag['self'])?'1':'0';
	...
	public function _catpos($attr) {
		...
		$format		= !empty($tag['format']) ? $tag['format'] : '';
		...
		$space=str_replace("}",">",$space);
 		...
 		if(is_numeric($tag['catid'])){
			...
			foreach($arrparentid as $cid) {
				$parsestr[] = str_replace("$",'<a href="'.$category_arr[$cid]['url'].'">'.$category_arr[$cid]['catname'].'</a>',$format);
			}unset($category_arr);
			...
			$parsestr .= '$urlt=\'<a href="\'.$Categorys[$cid][\'url\'].\'">\'.$Categorys[$cid][\'catname\'].\'</a>\';';
			$parsestr .= '$parsestr[] = str_replace("$",$urlt,\''.$format.'\');?>';
	...
	public function _list($attr,$content) {
			...
			{   //根据用户输入的值拼接查询条件
				...
				$field  = isset($tag['field'])?$tag['field']:'id,catid,url,title,keyword,description,image,createtime';
